...
with webdriver.Chrome() as browser:
    ...
    result_dict = {}
    age, count_skills = 100, 0
    for dict_cookie in cookies:
        browser.delete_all_cookies()
        browser.add_cookie({'name': dict_cookie['name'], 'value': dict_cookie['value']})
        browser.refresh()
        time.sleep(0.5)

        tmp_age = int(browser.find_element(By.ID, 'age').text.split(':')[1])
        tmp_count_skills = len(browser.find_element(By.ID, 'skillsList').find_elements(By.TAG_NAME, 'li'))

        if tmp_age < age and tmp_count_skills > count_skills:
            result_dict = dict_cookie
            age = tmp_age
            count_skills = tmp_count_skills
            print(f'New best result: {result_dict}')
