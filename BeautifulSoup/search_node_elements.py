...
def calculate_total_price(html: str) -> float:
    ...
    prices_stocks = soup.find_all(class_=['count price','count stock'])
    total_price=[]
    for i in range(len(prices_stocks)):
        if i%2==0:
            price = float(prices_stocks[i].text.lstrip('Цена: $'),)
        else:
            stock = int(prices_stocks[i].text.lstrip('Количество на складе: '))
            total_number =float(price*stock)
            total_price.append(total_number)
